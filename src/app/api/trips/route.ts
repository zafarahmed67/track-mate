...
async function processTripGenerationJob(job: TripGenerationJob): Promise<void> {
  ...
  const suggestedDays = Math.max(
    1,
    Math.round(estimatedDriveDistanceKm / kmPerDay),
  );
  console.log(
  ...
  if (daysAdjusted) {
    const requestedKmPerDay = Math.round(
      estimatedDriveDistanceKm / job.tripDurationDays,
    );
    const adjustedKmPerDay = Math.round(
      estimatedDriveDistanceKm / finalTripDays,
    );
    daysAdjustment = {
  ...
  const organizationDistanceKm =
    customRouteDistanceKm ?? estimatedDriveDistanceKm;
